...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        'tw:border-border/50 tw:grid tw:min-w-[8rem] tw:items-start tw:gap-1.5 tw:rounded-lg tw:border tw:border-[var(--borderColor-muted)] tw:bg-default tw:px-2.5 tw:py-1.5 tw:text-xs tw:shadow-xl',
        className
      )}
    >
        ...
        {payload.map((item, index) => {
          ...
          return (
            <div
              key={item.dataKey}
              className={cn(
                'tw:[&>svg]:text-muted tw:flex tw:w-full tw:flex-wrap tw:items-stretch tw:gap-2 tw:[&>svg]:h-2.5 tw:[&>svg]:w-2.5',
              ...
              {formatter && item?.value !== undefined && item.name ? (
                ...
                <>
                  ...
                  >
                    ...
                    {item.value && (
                      <span className='tw:text-default tw:font-mono tw:font-medium tw:tabular-nums'>
      ...
      {payload.map((item) => {
        ...
        return (
          <div
            key={item.value}
            className={cn(
              'tw:[&>svg]:text-muted tw:flex tw:items-center tw:gap-1.5 tw:[&>svg]:h-3 tw:[&>svg]:w-3'
            )}
          >
